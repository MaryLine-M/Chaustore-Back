			...
			<?php 
				...
				$id = checkInput($_GET['id']);
					...
					if(!empty($_POST)){
						...
						$req = "DELETE FROM size WHERE id = $id";
		
						$sup = mysqli_query($connect, $req);
		
						if ($sup){
							header("Location: sizes.php?delsuccess=1");
						} else {
							echo '<p class="alert"> Une erreur est survenue impossible de supprimer cet élément</p>';}
					}
			?>
				
			<form class="" role="form" action="deleteSize.php" method="post">
			
				<input type="hidden" name="id" value="<?php echo $id; ?>"/>
				
				<p class="alert">Êtes-vous sûr de vouloir supprimer cette taille ?</p>
